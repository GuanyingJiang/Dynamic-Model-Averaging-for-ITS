...
# DYNAMIC MODEL AVERAGING WITH UNKNOWN OBSERVATIONAL VARIANCE
def dynamic_model_averaging_uov(K, T, T0, h_fore, V_0,
	...
	for irep in range(T):
		...
		for k in range(K):
			...
			y_t_pred[k].append(np.matrix(x_t[irep,:]) * theta_pred[k])
			...
			if irep == 0:
				...
				V_t_1 = V_t[k][irep-1]
				V_t[k].append(V_t_1 + V_t_1 / irep * (A_t[k]/(V_t_1+xRx2) - 1))
			...
			# and evaluate at the future dependent value a Student's t density.
			...
			f_l = sst.pdf(y_t[irep], df=irep+1, loc=mean, scale=np.sqrt(variance[k][irep]))
